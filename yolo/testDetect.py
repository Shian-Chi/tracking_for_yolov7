...
from myProject.motor import motorEcho, motorSend, motorRecv
...
def isTracked(x, y):
    e1, e2 = None, None
    if int(x) == int(y) == int(0):
        e1 = motor1.encoder2angle()
        time.sleep(0.0001)
        e2 = motor2.encoder2angle()
    return e1, e2
...
def motorPID_Ctrl_1(frameCenter_X, frameCenter_Y):
    Row_PID_ErrVal = int(rowPID.run(frameCenter_X)*100)
    Pitch_PID_ErrVal = int(pitchPID.run(frameCenter_Y)*100)

    pidErr = pitchPID.run(frameCenter_X, frameCenter_Y)
    data1 = motor1.IncrementTurnVal(Row_PID_ErrVal)
    motorSend(data1, 10)
    time.sleep(1E-6)
    data2 = motor2.IncrementTurnVal(Pitch_PID_ErrVal)
    motorSend(data2, 10)
    print(f"{pidErr[0]:.3f}", f"{pidErr[1]:.3f}")

    # print("angle1: ",encoder1/91,"angle2: ",encoder2/91)
    # if ser.write((str(pidErr[0])+', '+str(angle1)+', '+str(pidErr[1])+', '+str(angle2)+'\n').encode("utf-8")):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    # Check if the class label is "tennis"
                    if names[int(cls)] == "tennis-ball":
                        if conf > max_conf:
                            max_conf = conf
                            max_xyxy = xyxy
                    """
                ...
                PID(max_xyxy)
            """
            if trackTime(t1, t2, t3) == 0:
                dist.getDistance(max((max_xyxy[0]-max_xyxy[2]).item()), max(max_xyxy[1]-max_xyxy[3].item()))
            """
            
            # Print time (inference + NMS)
            print(f'{s}Done. ({(1E3*(t2-t1)):.1f}ms) Inference, ({(1E3*(t3-t2)):.1f}ms) NMS, FPS:{1E3/((1E3*(t2-t1))+(1E3*(t3-t2))+s_t):.1f}\n')
