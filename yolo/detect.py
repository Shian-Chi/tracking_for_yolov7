...
def PID(xyxy):
    if xyxy is not None:
        ...
        motorPID_Ctrl(((xyxy[0] + xyxy[2]) / 2).item(), ((xyxy[1] + xyxy[3]) / 2).item())


def detect(save_img=False):
    source, weights, view_img, save_txt, imgsz, trace, track = opt.source, opt.weights, opt.view_img, opt.save_txt, opt.img_size, not opt.no_trace, opt.track
...
def parse_args():
    ...
    parser.add_argument('--track', default='tennis-ball', help='track object')
